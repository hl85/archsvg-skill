...
> 📐 **超长文案自动折行**（盒宽上限 240 → 行宽上限 212）：
> 节点标题/副标签超过行宽上限时按行拆多行（`lib/layout.mjs` 的 `wrapText`；CJK 逐字断、
> 拉丁按空格断、单个超长词硬断；按**最小行数均分**，避免末行只剩孤字）。
> 盒高按行数计：1 行 42、2 行 58、每多 1 行 +18（`LINE_H`，与 `render.mjs` 一致）。
> **同带节点按行内垂直居中**（共享同一 `cy`）——这样带内水平直连边恒为直线
> （行高不一时 Δcy 会产生 <16px 竖段，触发 `route_rhythm`）。

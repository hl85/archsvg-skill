...
| 用途 | 取色 | 例子（control 紫） |
|:---|:---|:---|
| 节点 / 组框填充 | 50 级极浅（亮色） | `#fdf4ff` |
| **卡片描边** | **600 级深色** | `#7e22ce` |
| 文字（若用 role 色） | 600 级深色 | `#7e22ce` |

> ⚠️ **描边必须取 600 级**。历史上的描边是 400–500 级饱和色（如 `#a855f7`），
> 实测「描边 vs 画布 `#f8fafc`」对比度只有 **2.18–3.78**，卡片轮廓立不起来、放大后发飘。
> 亮色下 5 个 role 的描边**直接复用其 600 级文字色**（不新增色值），实测对比度：
>
> | role | 描边 | vs 画布 |
> |:---|:---|---:|
> | control | `#7e22ce` | 6.67 |
...
> | neutral | `#334155` | 9.90 |
>
> 暗色描边沿用原有暗色 role stroke，不动。

**组框着域色**：`frames[].role` 会被渲染成 `class="frame frame-{role}"`，不再统一用灰白面板。

- **亮色填充** = 该 role 的 `fill` 向画布混 **50%**（比节点浅一档，避免组框与节点融成一片）；
  混色在生成 CSS 变量处用代码算出（`theme.mjs` 的 `frameFill()`），不手写 5 个新 hex。
- **暗色填充** = 该 role 暗色 `fill` 降到 **alpha 0.10** 的半透明域色（不为 5 个域另造实底 hex；
  800 级实底会让暗色图变重）；
- **描边** = 该 role 的 600 级色，与节点描边**同源**。

护栏：`theme_readable` 同时断言「frame 描边 vs 合成后的 frame 填充 ≥ 3:1」（WCAG 1.4.11 非文本对比度），
明暗各一套 —— 否则组框边界会糊掉，而只看文字的对比度检查查不出来。
...
> 另注：卡片/组框的描边取 600 级深色，色阶与组框着域色的规则见 §1.1。

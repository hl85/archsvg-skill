...
// frames[].role，语义白白丢掉。此处让组框着域色，与节点同源、只做「更淡一层」：
//   - 亮色填充：该 role 的 fill 向画布混 50%（比节点浅一档，避免组框与节点融在一起）；
//   - 暗色填充：沿用半透明域色思路，把该 role 的暗色 fill 的 alpha 降到 0.10
//     （不为 5 个域新造 5 组实底 hex；800 实底会让暗色图变重，破坏现有克制美学）；
//   - 描边：与该 role 的节点描边同源（亮色 600 级 / 暗色 role stroke），不另写常量。
...
export function frameFill(mode, role) {
  ...
  const bg = parseColor(t.canvas);
  const mix = (i) => Math.round(fg[i] * FRAME_FILL_MIX + bg[i] * (1 - FRAME_FILL_MIX));
  return rgbHex(mix(0), mix(1), mix(2));
}

export function frameStroke(mode, role) {
  const t = mode === 'dark' ? TOKENS.dark : TOKENS.light;
  return t.roles[role] ? t.roles[role].stroke : null;
}
...
function varsLines(t, mode, indent = '') {
  ...
  for (const r of ROLE_KEYS) {
    ...
    out.push(`${indent}--frame-${r}-stroke: ${frameStroke(mode, r)};`);
  }
  return out;
}
...
function baseCss() {
  ...
  for (const r of ROLE_KEYS) {
    ...
    // 组框着域色：必须排在 `.frame` 之后（同优先级、后者生效）才能覆盖灰白面板。
    roleRules.push(`.frame-${r} { fill: var(--frame-${r}-fill); stroke: var(--frame-${r}-stroke); }`);
  }
  return [

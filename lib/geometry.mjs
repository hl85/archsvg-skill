...
// 共线 / 退化 / 阈值带宽容差。
const EPS = 0.0001;
// 仅用于「线段是否退化成一个点」与端点共线判定。
const EPS7 = 0.0000001;

// 点—线段投影的退化判定阈值。注意这是**平方**长度的阈值，
// 等效长度阈值 = sqrt(1e-7) ≈ 3.16e-4；与参考行为一致（它比较的就是平方值）。
const DEGENERATE_LENGTH_SQ = 1e-7;
...
function pointSegmentDistance(px, py, a, b) {
  ...
  if (lengthSq <= DEGENERATE_LENGTH_SQ) return Math.hypot(px - a[0], py - a[1]);

...
import { contrastRatio, TOKENS, frameFill, frameStroke } from '../theme.mjs';
...
// ============ 5. theme_readable ============
// 两块断言：
//   ① 文字可读：每个 role 的 text 与 fill 对比度 ≥ 4.5:1（WCAG 1.4.3），明/暗各一套；
//   ② 组框轮廓可辨：frame 的 stroke 与其合成后的 fill 对比度 ≥ 3:1（WCAG 1.4.11 非文本对比度）。
//      组框现在着域色，若描边与填充太近，容器边界会「糊」掉 —— 而①只看文字，查不出来。
function checkThemeReadable(ir) {
  ...
  for (const role of used) {
    ...
    if (lightRatio < MIN_TEXT) {
      fails.push(`role ${role} 亮色：text ${lt.text} 与 fill ${lt.fill} 对比度 ${lightRatio.toFixed(2)} < ${MIN_TEXT}`);
    }
    ...
    if (darkRatio < MIN_TEXT) {
      fails.push(`role ${role} 暗色：text ${dt.text} 与合成底色 ${darkBg} 对比度 ${darkRatio.toFixed(2)} < ${MIN_TEXT}`);
    }

    // ② 组框：stroke vs 合成后的 frame fill。亮色 fill 已是向画布混色的不透明 hex；
    //    暗色 fill 是 alpha 0.10 的 rgba，需先与暗色画布合成。
    const lf = frameFill('light', role);
    const ls = frameStroke('light', role);
    const lfRatio = contrastRatio(ls, lf);
    if (lfRatio < MIN_FRAME) {
      fails.push(`role ${role} 亮色组框：stroke ${ls} 与 frame fill ${lf} 对比度 ${lfRatio.toFixed(2)} < ${MIN_FRAME}（组框轮廓会糊）`);
    }
    const df = compositeHex(frameFill('dark', role), TOKENS.dark.canvas);
    const ds = frameStroke('dark', role);
    const dfRatio = contrastRatio(ds, df);
    if (dfRatio < MIN_FRAME) {
      fails.push(`role ${role} 暗色组框：stroke ${ds} 与合成 frame fill ${df} 对比度 ${dfRatio.toFixed(2)} < ${MIN_FRAME}（组框轮廓会糊）`);
    }
  }
  ...
  return {
    ...
    ok,
    details: ok
      ? [`明/暗两套 token 下，用到的 ${used.size} 个 role 文字对比度均 ≥ ${MIN_TEXT}:1，组框描边/填充对比度均 ≥ ${MIN_FRAME}:1`]
      : fails,
  };
}

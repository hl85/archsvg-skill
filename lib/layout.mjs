...
function finalize({ type, boxes, frames, edges, vb }) {
  ...
  if (rolesUsed.size >= 2) {
    ...
    const lh = 18;
    let lw = 16;
    for (const en of entries) lw += sw + 8 + estimateTextWidth(en.label, 12) + 10;
    lw = Math.max(lw, 120);
  ...
  if (frames.length > 1) {
    ...
    for (const band of nonGrid) {
      ...
      if (!xs) {
        ...
        const gap = n > 1 ? Math.max((innerW - sumW) / (n - 1), GAP) : 0;
        let x = n > 1 ? innerLeft : innerLeft + (innerW - sumW) / 2;
        xs = bs.map((b) => {

...
const FRAME_PAD = 18;
const STUB = 14;
// 标签净空阈值：与 checks/composition.mjs 中 Math.max(8, LABEL_HEIGHT) 一致（LABEL_HEIGHT=14）。
const LABEL_FONT = 11;
const LABEL_H = 14;
const LABEL_PAD = 8;
const LABEL_MIN_CLEAR = 14;
...
function boxSize(node) {
  const labelW = estimateTextWidth(node.label || '', NODE_FONT);
  const subW = node.sublabel ? estimateTextWidth(node.sublabel, SUB_FONT) : 0;
  const w = Math.min(240, Math.max(96, Math.max(labelW, subW) + 2 * BOX_PAD_X));
  const h = node.sublabel ? 46 : 34;
...
function finalize({ type, boxes, frames, edges, vb }) {
  ...
  width = Math.max(vb[0], maxX + MARGIN);
  height = Math.max(vb[1], maxY + MARGIN);

  // legend：用到的 role >= 2 时生成，置于右下角，必要时下移避免重叠。
  ...
  if (rolesUsed.size >= 2) {
    ...
    const lh = 18;
    let lw = 16;
    for (const en of entries) lw += sw + 8 + estimateTextWidth(en.label, 11) + 10;
    lw = Math.max(lw, 120);
    const lhTotal = entries.length * lh + 12;
    let lx = width - MARGIN - lw;
    let ly = height - MARGIN - lhTotal;
    const lrect = { x: lx, y: ly, width: lw, height: lhTotal };
    const overlaps = [...boxes, ...frames].some((r) => rectsOverlap(r, lrect, 8));
    if (overlaps) {
      ly = maxY + 16;
      lx = width - MARGIN - lw;
      height = Math.max(height, ly + lhTotal + MARGIN);
    }
    legend = {
      x: Math.round(lx),
      y: Math.round(ly),
...
export function layoutArchitecture(ir) {
  ...
  for (const e of edges) {
    ...
    let fromSide;
    let toSide;
    const fromBand = groups.length ? bands.find((b) => b.nodeIds.includes(e.from)) : null;
    const toBand = groups.length ? bands.find((b) => b.nodeIds.includes(e.to)) : null;
    if (fromBand && toBand && fromBand.id === toBand.id) {
      ...
      const fy = fromBand ? bandRects.get(fromBand.id).top : fb.y;
      const ty = toBand ? bandRects.get(toBand.id).top : tb.y;
      if (ty >= fy) { fromSide = 'bottom'; toSide = 'top'; } else { fromSide = 'top'; toSide = 'bottom'; }
    }

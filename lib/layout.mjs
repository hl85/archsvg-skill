...
import { rectsOverlap, collectLabelRouteClearance } from './geometry.mjs';
...
const FRAME_PAD = 18;
const STUB = 14;
// 标签净空阈值：必须 >= checks/composition.mjs 中 Math.max(8, LABEL_HEIGHT)（LABEL_HEIGHT=15），
// 否则放置器会选出「刚好 0.5px 不达标」的位置，导致 label_route_clearance 误报。
const LABEL_FONT = 12;
const LABEL_H = 15;
const LABEL_PAD = 8;
const LABEL_MIN_CLEAR = 15;
// 回边外侧绕行通道与最左 box 左缘的间距。
const CHANNEL_GAP = 28;
// 阶段带（stages）列间距：横向演进图里相邻档位之间的边是水平段，
// 间距必须大于 2*STUB 才能留出足够长的内部段（route_rhythm 要求 interior >= 16px）。
const STAGE_COL_GAP = 64;
...
// 估算文字宽度：中文 1em/字，英文数字约 0.55em/字。
export function estimateTextWidth(text, px) {
  if (!text) return 0;
  let w = 0;
  for (const ch of String(text)) w += isCjkChar(ch.codePointAt(0)) ? px : px * 0.55;
  return w;
}

// 文本折行：超长文本按「估算宽度」拆成多行（均分：按最少行数取目标行宽，避免末行只剩孤字）。
// CJK 逐字可断；拉丁按空格断；单个超长 token 再逐字符硬断。
export function wrapText(text, px, maxW) {
  const s = String(text || '');
  if (!s) return [''];
  const total = estimateTextWidth(s, px);
  if (total <= maxW) return [s];
  const limit = Math.min(maxW, total / Math.ceil(total / maxW));
  const tokens = [];
  let buf = '';
  for (const ch of s) {
    if (isCjkChar(ch.codePointAt(0))) {
  ...
  for (const tok of tokens) {
    if (cur !== '' && estimateTextWidth(cur + tok, px) > limit) flush();
...
const MAX_CONTENT_W = 240 - 2 * BOX_PAD_X;
// 每行行高（必须与 render.mjs 的 renderBoxes 一致）。
const LINE_H = 18;

function boxSize(node) {
  const labelLines = wrapText(node.label || '', NODE_FONT, MAX_CONTENT_W);
  const subLines = node.sublabel ? wrapText(node.sublabel, SUB_FONT, MAX_CONTENT_W) : [];
  const labelW = Math.max(0, ...labelLines.map((l) => estimateTextWidth(l, NODE_FONT)));
  const subW = Math.max(0, ...subLines.map((l) => estimateTextWidth(l, SUB_FONT)));
  const w = Math.min(240, Math.max(96, Math.max(labelW, subW) + 2 * BOX_PAD_X));
  const rows = Math.max(1, labelLines.length + subLines.length);
  const h = rows <= 1 ? 42 : 58 + (rows - 2) * LINE_H;
  return { width: Math.round(w), height: h, labelLines, subLines };
}
...
function finalize({ type, boxes, frames, edges, vb }) {
  ...
  // 先按当前边界放置标签，再据标签位置扩边界。
  placeLabels(edges, boxes, width, height);
  for (const e of edges) if (e.labelAt) acc(e.labelAt[0], e.labelAt[1]);
  ...
  if (rolesUsed.size >= 2) {
    ...
    const colW = Math.max(...entries.map((en) => sw + 8 + estimateTextWidth(en.label, 12) + 10));
    const lw = Math.max(16 + cols * colW + (cols - 1) * colGap, 120);
    const lhTotal = rows * lh + 12;
    const lx = contentRight - lw;
    const ly = contentBottom + 16;
    legend = {
      x: Math.round(lx),
      y: Math.round(ly),
      width: Math.round(lw),
...
export function layoutArchitecture(ir) {
  ...
  for (const band of bands) {
    ...
    if (band.grid) {
      ...
      for (let r = 0; r < rows; r += 1) {
        ...
        for (let c = 0; c < cols; c += 1) {
          const idx = r * cols + c;
          if (idx >= n) break;
          const b = makeBox(bNodes[idx], cx + (colW[c] - sizes[idx].width) / 2, cy + (rowH[r] - sizes[idx].height) / 2);
          boxById.set(b.id, b);
          cx += colW[c] + GAP;
        }
        cy += rowH[r] + GAP;
    ...
    for (let i = 0; i < bNodes.length; i += 1) {
      // 行内垂直居中：使同带各节点共享同一 cy —— 带内水平直连边才不会因行高不一而出现 <16px 的竖段（route_rhythm）。
      const b = makeBox(bNodes[i], cx, boxY + (rowH - sizes[i].height) / 2);
      boxById.set(b.id, b);
...
export function layoutFlow(ir) {
  ...
  if (stages.length) {
    ...
    for (const st of stages) {
      ...
      const w = Math.max(160, Math.max(...sizes.map((s) => s.width), 0));
...
export function layoutSequence(ir) {
  ...
  const lifeTop = TITLE_H + 40;
  const rowGap = 44;
  const msgTop = lifeTop + 12;
  const edges = [];
  let y = msgTop;

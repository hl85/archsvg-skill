...
- **ε₇ = 0.0000001（1e-7）**：用于「线段是否退化成一个点」以及点—线段投影的退化判断；其可见效果是：退化线段按点处理。
  ⚠️ 注意它在**点—线段距离**这一处是拿**长度的平方**与 1e-7 比较（不是拿长度本身），
  因此该处的等效长度阈值是 `sqrt(1e-7) ≈ 3.16e-4`。两个量级的差别在段长落于
  `(1e-7, 3.16e-4]` 时可见（语料 `syn@lead@…:degenerate-seg-*` 三条专门钉住这条边界）。
